class ApitoolkitGrailsPlugin {
    def version = "0.9"
    def grailsVersion = "2.3 > *"
    def title = "Grails API Toolkit"
    def author = "Owen Rubel"
    def authorEmail = "orubel@gmail.com"
    def description = 'The Grails API Toolkit is a set of tools that automate alot of the tasks needed to build your API by combining the functionality of REST, RPC and HATEOAS.'
    def documentation = "https://github.com/orubel/grails-api-toolkit"

    def license = "Apache"
    def issueManagement = [system: 'GitHub', url: 'https://github.com/orubel/grails-api-toolkitissues']
    def scm = [url: 'https://github.com/orubel/grails-api-toolkit']
}
